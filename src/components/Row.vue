<template>
    <tr
        ...
        ref="vbt_row"
        v-bind:style="{
            background: rowHiglighted ? highlightRowHoverColor : '',
        }"
        :class="rowClasses"
        v-on="
            rowsSelectable ? { click: ($event) => handleRowSelect($event) } : {}
        "
    >
        <CheckBox
            v-if="checkboxRows"
...
export default {
    ...
    methods: {
        ...
        checkInSelecteditems(selectedItems, row) {
            ...
            if (this.uniqueId) {
                const item = selectedItems.filter((selectedItem) => {
                    if (selectedItem[this.uniqueId]) {
                        return (
                            selectedItem[this.uniqueId] === row[this.uniqueId]
                        );
                    }
                });
